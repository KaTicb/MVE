...
int mane(int argc, char **argv) {
  ...
  while (k < rp && prz == 1) {
    ...
    for (j = 0; j < j2; j++) {
      for (i = 1; i <= i2; i++) {
        t0 = T[j][i];
        if (i >= 0 && i < i2 && j == 0) {
          tx = -(alf_3 * (T[j][i - 1] + T[j][i + 1]) + alf_1 * T[j + 1][i]) /
               gam_3;
          del = lam * (tx - t0);
          t1 = t0 + del;
          T[j][i] = t1;
          ...
          tx = -(alf_4 * (T[j - 1][i] + T[j + 1][i]) + alf_2 * T[j][i - 1]) /
               gam_3;
          del = lam * (tx - t0);
          t1 = t0 + del;
          T[j][i] = t1;
        ...
        if (i > 0 && i < i1 && j == j1) {
          ...
          tx = -(alf_3 * T[j][i - 1] + alf_4 * T[j + 1][i] +
                 alf_2 * T[j][i + 1] + alf_1 * T[j1][i]) /
               gam_2;
          del = lam * (tx - t0);
          t1 = t0 + del;
          T[j][i] = t1;
          ...
          tx = -(alf_1 * (T[j - 1][i] + T[j + 1][i]) +
                 alf_2 * (T[j][i - 1] + T[j][i + 1])) /
               gam_1;
          del = lam * (tx - t0);
          t1 = t0 + del;
          T[j][i] = t1;
  ...
  foutT.close();
  ofstream fouT("c:/work/nT.dat",
                ios_base::out | ios_base::binary);

  fouT.write((char *)&nT, sizeof nT);
  fouT.close(); // закрываем файл
  ofstream fout("c:/work/Pole.dat",
                ios_base::out | ios_base::trunc | ios_base::binary);
